...
func getElectricalProps(ioType string) pinSpec {
	props := pinSpec{}
	ioType = strings.Split(ioType, "_")[0]
	switch ioType {
	case "TTa", "TT":
		props = pinSpec{
			Vih: &ValueOptions{
...
var (
	...
	uarttxRE  = regexp.MustCompile(`USART\d_TX`)
	uartrxRE  = regexp.MustCompile(`USART\d_RX`)
	uartRE    = regexp.MustCompile(`USART.*`)
	i2csdaRE  = regexp.MustCompile(`I2C\d_SDA`)
	i2csclRE  = regexp.MustCompile(`I2C\d_SCL`)
	i2cRE     = regexp.MustCompile(`I2C.*`)
	jtagRE    = regexp.MustCompile(`JT.*`)
	spicopiRE = regexp.MustCompile(`SPI\d_MOSI.*`)
	spicipoRE = regexp.MustCompile(`SPI\d_MISO.*`)
	spiclkRE  = regexp.MustCompile(`SPI\d_SCK.*`)
	spicsRE   = regexp.MustCompile(`SPI\d_NSS.*`)
				...
				Values: []Value{
					{
						SiUnit:     "megahertz",
						TypValue:   8,
						Conditions: []string{"High-speed internal (HSI) RC oscillator"},
					},
				},
			},
